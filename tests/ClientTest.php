...
class ClientTest extends TestCase
{
    ...
    public function testListRevisions(): void
    {
        ...
        $response = $this->client->listRevisions($gdFile['id']);

        $this->assertArrayHasKey('revisions', $response);
        $revisions = $response['revisions'];
        $this->assertCount(2, $revisions);
        $this->assertArrayHasKey('kind', $revisions[0]);
        $this->assertArrayHasKey('id', $revisions[0]);
        $this->assertEquals('drive#revision', $revisions[0]['kind']);
    ...
    public function testGetRevision(): void
    {
        ...
        $response = $this->client->listRevisions($gdFile['id']);
        $lastRevision = array_pop($response['revisions']);
        $response = $this->client->getRevision($gdFile['id'], $lastRevision['id']);

        $this->assertArrayHasKey('kind', $response);
        $this->assertArrayHasKey('id', $response);
        $this->assertEquals('drive#revision', $response['kind']);
        $this->assertEquals($lastRevision['id'], $response['id']);
    }

    public function testUpdateRevision(): void
    {
        $gdFile = $this->client->createFile(
            $this->dataPath . '/titanic.csv',
            'titanic',
        ...
        $values = $this->csvToArray($this->dataPath . '/titanic_2.csv');
        $this->client->clearSpreadsheetValues($gdFile['id'], 'titanic');
        $this->client->updateSpreadsheetValues($gdFile['id'], 'titanic', $values);

        $response = $this->client->listRevisions($gdFile['id']);
        $revisions = $response['revisions'];
        $previousRevision = $revisions[2];

        // publish the previous revision
        $this->client->updateRevision($gdFile['id'], $previousRevision['id'], [
            'published' => true,
        ]);

        $revision = $this->client->getRevision($gdFile['id'], $previousRevision['id']);
        $this->assertTrue($revision['published']);
